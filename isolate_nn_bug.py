...
def test_network(
    ...
    if total_grad < 1e-6:
        print("    ⚠️  WARNING: Gradients are near ZERO!")
    ...
    if best_auc > 0.60:
        print("  ✅ NETWORK CAN LEARN!")
    else:
        print("  ❌ NETWORK CANNOT LEARN")
...
def main():
    ...
    for name, res in results.items():
        ...
        print(
            f"{name:<50} {res['auc']:>10.4f} {res['best_auc']:>10.4f} {status:>10}"
        )
    ...
    if results["simple_mlp"]["best_auc"] > 0.60:
        if results["groupnorm"]["best_auc"] < 0.55:
            ...
            results["simple_no_pw"]["best_auc"] > 0.60
            and results["groupnorm"]["best_auc"] < 0.55
        ):
